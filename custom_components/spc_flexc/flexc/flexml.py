...
def build_zone_status_command(zone_id: int, username: str, password: str) -> str:
    ...
    return _build_command_envelope(f'<CMD_GET_ZONE_STATUS ZONE_ID="{zone_id}" />', username, password)
...
def build_zone_control_command(zone_id: int, action: int, username: str, password: str) -> str:
    ...
    return _build_command_envelope(f'<CMD_ZONE_CONTROL ZONE_ID="{zone_id}" ACTION="{action}" />', username, password)
...
def build_door_control_command(door_id: int, action: int, username: str, password: str) -> str:
    ...
    return _build_command_envelope(f'<CMD_DOOR_CONTROL DOOR_ID="{door_id}" ACTION="{action}" />', username, password)
...
def parse_zone_control(response: str, zone_id: int) -> None:
    """Validate the real-panel REPLY_ZONE_CONTROL response."""
    root = _parse_reply_root(response)
...
def parse_door_control(response: str) -> None:
    """Validate the empty successful REPLY_DOOR_CONTROL captured on SPC4300."""
    root = _parse_reply_root(response)
