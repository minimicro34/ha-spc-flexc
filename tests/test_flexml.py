...
def test_zone_control_builds_validated_actions() -> None:
    ...
    inhibit = build_zone_control_command(1, 0, "HomeAssistant", "MyPassword")
    deinhibit = build_zone_control_command(1, 1, "HomeAssistant", "MyPassword")
    assert '<CMD_ZONE_CONTROL ZONE_ID="1" ACTION="0" />' in inhibit
    assert '<CMD_ZONE_CONTROL ZONE_ID="1" ACTION="1" />' in deinhibit
...
def test_parse_zone_control_real_panel_reply() -> None:
    ...
    response = (
        ...
        '<REPLY_ZONE_CONTROL RESULT="0" CMD_RESULT="OK">'
        '<ZONE_CONTROL ZONE_ID="1" RESULT="0"/>'
        "</REPLY_ZONE_CONTROL>"
        "</FLEXML_REPLY>"
    )
...
def test_parse_zone_control_rejects_wrong_zone() -> None:
    ...
    response = (
        '<FLEXML_REPLY VER="1.0">'
        '<REPLY_ZONE_CONTROL RESULT="0" CMD_RESULT="OK">'
        '<ZONE_CONTROL ZONE_ID="2" RESULT="0"/>'
        "</REPLY_ZONE_CONTROL>"
        "</FLEXML_REPLY>"
    )
...
def test_parse_zone_control_rejects_result_error() -> None:
    ...
    response = (
        '<FLEXML_REPLY VER="1.0">'
        '<REPLY_ZONE_CONTROL RESULT="0" CMD_RESULT="OK">'
        '<ZONE_CONTROL ZONE_ID="1" RESULT="54"/>'
        "</REPLY_ZONE_CONTROL>"
        "</FLEXML_REPLY>"
    )
...
def test_parse_empty_alert_status() -> None:
    ...
    response = (
        '<FLEXML_REPLY VER="1.0">'
        '<REPLY_GET_ALERT_STATUS RESULT="0" CMD_RESULT="OK">'
...
def test_zone_discovery_does_not_stop_on_empty_ok() -> None:
    ...
    response = (
        ...
        '<ZONE_STATUS ZONE_ID="5" ZONE_NAME="Zone 5" />'
        "</REPLY_GET_ZONE_STATUS>"
        '<REPLY_GET_ZONE_STATUS RESULT="0" CMD_RESULT="OK" />'
        '<REPLY_GET_ZONE_STATUS RESULT="0" CMD_RESULT="OK">'
        '<ZONE_STATUS ZONE_ID="7" ZONE_NAME="Sirene" />'
        "</REPLY_GET_ZONE_STATUS>"
        "</FLEXML_REPLY>"
    )
...
def test_door_discovery_detects_result_102() -> None:
    ...
    response = (
        '<FLEXML_REPLY VER="1.0">'
        '<REPLY_GET_DOOR_STATUS RESULT="0" CMD_RESULT="OK" />'
        '<REPLY_GET_DOOR_STATUS RESULT="102" CMD_RESULT="ERROR" />'
        "</FLEXML_REPLY>"
    )
