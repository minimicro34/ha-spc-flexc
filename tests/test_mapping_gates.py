...
@pytest.mark.asyncio
async def test_set_mapping_gate_recovers_verification_timeout_then_confirms() -> None:
    """A verification timeout recovers the session without surfacing a false failure."""
    coordinator = MagicMock()
    coordinator.state = SpcState(
        mapping_gates={1: MappingGateState(mg_id=1, state=False)}
    )
    ...
    coordinator.client.async_recover_session = AsyncMock()
    control_reply = (
        '<FLEXML_REPLY VER="1.0"><REPLY_MG_CONTROL RESULT="0" CMD_RESULT="OK">'
        '<MG_CONTROL MG_ID="1" RESULT="0" /></REPLY_MG_CONTROL></FLEXML_REPLY>'
    )
    status_on = (
        '<FLEXML_REPLY VER="1.0"><REPLY_GET_MG_STATUS RESULT="0" CMD_RESULT="OK">'
        '<MG_STATUS MG_ID="1" STATE="1" /></REPLY_GET_MG_STATUS></FLEXML_REPLY>'
    )
    coordinator.client.async_send_flexml = AsyncMock(
        side_effect=[control_reply, FlexCCommandTimeout("timeout"), status_on]
    )
    coordinator._update_mapping_gate_states = lambda raw: (
        SpcFlexCMappingGateCoordinator._update_mapping_gate_states(coordinator, raw)
    )
...
async def test_set_mapping_gate_retries_after_verification_timeout_if_still_off() -> None:
    ...
    status_off = (
        '<FLEXML_REPLY VER="1.0"><REPLY_GET_MG_STATUS RESULT="0" CMD_RESULT="OK">'
        '<MG_STATUS MG_ID="1" STATE="0" /></REPLY_GET_MG_STATUS></FLEXML_REPLY>'
    )
    status_on = (
        '<FLEXML_REPLY VER="1.0"><REPLY_GET_MG_STATUS RESULT="0" CMD_RESULT="OK">'
        '<MG_STATUS MG_ID="1" STATE="1" /></REPLY_GET_MG_STATUS></FLEXML_REPLY>'
    ...
    coordinator.client.async_recover_session.assert_awaited_once_with()
    assert coordinator.client.async_send_flexml.await_count == 5
    assert coordinator.state.mapping_gates[1].state is True
