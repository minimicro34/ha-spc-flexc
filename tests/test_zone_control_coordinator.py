...
async def test_inhibit_zone_refreshes_explicit_inhibition_state() -> None:
    coordinator = _coordinator_with_zone(
        ZoneState(zone_id=1, inhibit_allowed=True, raw={"ZONE_ID": "1", "INHIBIT_ALLOWED": "1"})
    )
    coordinator.client.async_get_zone_status = AsyncMock(return_value=[_zone_status(INHIBITED="1", DEINHIBIT_ALLOWED="1")])
...
async def test_isolate_zone_refreshes_explicit_isolation_state() -> None:
    ...
    coordinator = _coordinator_with_zone(
        ZoneState(zone_id=1, isolate_allowed=True, raw={"ZONE_ID": "1", "ISOLATE_ALLOWED": "1"})
    )
    coordinator.client.async_get_zone_status = AsyncMock(return_value=[_zone_status(ISOLATED="1", DEISOLATE_ALLOWED="1")])
...
async def test_isolate_zone_requires_explicit_permission() -> None:
    ...
    coordinator = _coordinator_with_zone(ZoneState(zone_id=1, isolate_allowed=None, raw={"ZONE_ID": "1"}))

    control = AsyncMock()
    with (
        patch("custom_components.spc_flexc.zone_control_coordinator.async_set_zone_isolated", new=control),
...
def _coordinator_with_zone(zone: ZoneState) -> MagicMock:
    ...
    coordinator._update_zone_from_control_status = lambda zone_id, raw: SpcFlexCZoneControlCoordinator._update_zone_from_control_status(coordinator, zone_id, raw)
    coordinator._async_refresh_zone_after_control = lambda zone_id, **kwargs: SpcFlexCZoneControlCoordinator._async_refresh_zone_after_control(coordinator, zone_id, **kwargs)
    return coordinator
